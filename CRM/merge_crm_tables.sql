...
INTO
	...
	WHERE
		...
		ON src.sourceid = tgt.SOURCEID
		AND src.OPPORTUNITYCOMPGUID = tgt.OPPORTUNITYCOMPGUID
	WHEN MATCHED THEN 
UPDATE
SET tgt.SOURCEID = src.SOURCEID,tgt.COMPETITORGUID = src.COMPETITORGUID,tgt.OPPORTUNITYCOMPGUID = src.OPPORTUNITYCOMPGUID,tgt.OPPORTUNITYGUID = src.OPPORTUNITYGUID,tgt.STATUS = src.STATUS,tgt.STATUSREASON = src.STATUSREASON,tgt.BRANDERPID = src.BRANDERPID,tgt.BRANDNAME = src.BRANDNAME,tgt.GRADENAME = src.GRADENAME,tgt.BUSINESSUNIT = src.BUSINESSUNIT,tgt.OPPCOMPCOMMENT = src.OPPCOMPCOMMENT,tgt.VOLUME = src.VOLUME,tgt.CREATEDON = src.CREATEDON,tgt.CREATEDBY = src.CREATEDBY,tgt.CREATEDRAVID = src.CREATEDRAVID,tgt.CREATEDPERSERPID = src.CREATEDPERSERPID,tgt.MODIFIEDON = src.MODIFIEDON,tgt.MODIFIEDBY = src.MODIFIEDBY,tgt.MODIFIEDRAVID = src.MODIFIEDRAVID,tgt.MODIFIEDPERSERPID = src.MODIFIEDPERSERPID,tgt.MAXCHANGEDDATE = src.MAXCHANGEDDATE	WHEN NOT MATCHED THEN
INSERT
	(SOURCEID,COMPETITORGUID,OPPORTUNITYCOMPGUID,OPPORTUNITYGUID,STATUS,STATUSREASON,BRANDERPID,BRANDNAME,GRADENAME,BUSINESSUNIT,OPPCOMPCOMMENT,VOLUME,CREATEDON,CREATEDBY,CREATEDRAVID,CREATEDPERSERPID,MODIFIEDON,MODIFIEDBY,MODIFIEDRAVID,MODIFIEDPERSERPID,MAXCHANGEDDATE) 
VALUES 
(src.SOURCEID,src.COMPETITORGUID,src.OPPORTUNITYCOMPGUID,src.OPPORTUNITYGUID,src.STATUS,src.STATUSREASON,src.BRANDERPID,src.BRANDNAME,src.GRADENAME,src.BUSINESSUNIT,src.OPPCOMPCOMMENT,src.VOLUME,src.CREATEDON,src.CREATEDBY,src.CREATEDRAVID,src.CREATEDPERSERPID,src.MODIFIEDON,src.MODIFIEDBY,src.MODIFIEDRAVID,src.MODIFIEDPERSERPID,src.MAXCHANGEDDATE);
...
INTO
	...
	WHERE
		...
		ON src.sourceid = tgt.SOURCEID
		AND src.SAMPLEGUID = tgt.SAMPLEGUID
	WHEN MATCHED THEN 
UPDATE
...
INTO
	...
	WHERE
				to_timestamp(to_char(MODIFIEDON,'YYYY-MM-DD-HH24:MI:SS.FF6'),'YYYY-MM-DD-HH24:MI:SS.FF6') >= (
		SELECT
			DATE(NVL(MAX(MODIFIEDON),TO_DATE('1000-01-01','YYYY-MM-DD')))
